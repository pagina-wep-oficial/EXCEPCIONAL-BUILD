...
db.from("client_requests").select("*").eq("project_id",id).order("created_at",{ascending:false}),
    ...
    if(pos===3){focus.innerHTML=`<div class="focus-icon">3</div><div><span>Lista para revisar</span><h2>Mira tu página antes de publicarla</h2><p>Revísala con calma. Si quieres cambiar algo, envíanos una solicitud.</p></div><span class="muted-box">La vista previa estará disponible en cuanto la activemos.</span>`;actionsCard.hidden=false;}
    if(pos===4){focus.innerHTML=`<div class="focus-icon done">✓</div><div><span>Proyecto publicado</span><h2>Tu página ya está en internet</h2><p>Puedes compartirla y pedir cambios o mantenimiento cuando lo necesites.</p></div>`;actionsCard.hidden=false;}
    }
